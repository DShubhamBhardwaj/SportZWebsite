         ...
         if($result->num_rows>0){
            $row = mysqli_fetch_row($result);
             ...
             if(!empty($newusername)){
                $sql1="UPDATE user SET username ='$newusername' WHERE  ID='$row[0]'";
                if($conn->query($sql1)){
                    echo'<script>
                        alert("username Upadated")
                        </script>
                        '; 
                }
            }
...
<div class="container-fluid bg">
    <div class="row">
        ...
        <form class="form-container" method="post" action="verified.php?username=<?php echo $username?>" > 
            <h1 style="color: #000066">Enter new Username: </h1>
  <div class="form-group">
    <label for="uname"><b>Username:</b></label>
    <input type="password" class="form-control" id="uname" placeholder="Enter Username" name="uname">
  </div>
...
<nav class="navbar navbar-expand-md navbar-dark bg-dark fixed-top">
  ...
  <a href="verified.php?username=<?php echo $username ?>">Change Username</a>
  <a href="password.php?username=<?php echo $username ?>">Change Passowrd</a>
</div>
