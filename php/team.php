...
<nav class="navbar navbar-expand-md navbar-dark bg-dark fixed-top">
    ...
    <main role="main" class="container">
        ...
        $Tname = filter_input(INPUT_GET,'name');
        $host="localhost";
        $dbusername="root";
        $dbpassword="";
        $dbname="sports";
        ...
        if(mysqli_connect_error()){
            ...
            $sql2="select Gname from games where gameID=$ID";
            $res=$conn->query($sql2);
            $Gname=mysqli_fetch_row($res);
            echo"<h1>$Gname[0] Team : $Tname</h1>";
           
            $sql1="select teamID from team where gameID='$ID' and Tname='$Tname' ";
            $tid=$conn->query($sql1);
            $Tid= mysqli_fetch_row($tid);
            
            $sql="select image,Pname,position,PRanking from player where gameID='$ID' and teamID='$Tid[0]' ";
            $result=$conn->query($sql);
            if($result->num_rows>0){
                while($row = mysqli_fetch_row($result))
                {
                   echo'
                   <tr><td><img src="data:image/jpeg;base64,'.base64_encode($row[0]).'"/></td> 
                    '; 
                    echo"<td><a href=player.php?username=$username&gameID=$ID&teamID=$Tid[0]&name=$row[1]> $row[1]</a> </td><td>$row[2]</td><td>$row[3]</td></tr>";
